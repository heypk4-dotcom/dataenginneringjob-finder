        ...
        for url in self.search_urls:
            try:
                ...
                if response.status_code != 200:
                    print(
                        f"LinkedIn scraper failed with status code {response.status_code}"
                    )
                    continue
                ...
                for card in job_cards:
                    ...
                    full_desc_text = (
                        f"{title_text} at {company_text} in {location_text}"
                    )
                    try:
                        time.sleep(1)  # Be polite when hitting individual job pages
                        job_res = requests.get(
                            apply_link, headers=self.headers, timeout=10
                        )
